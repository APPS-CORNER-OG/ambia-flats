       ...
       <div class="content-main">
  		...
  		<!--banner-->	
		    <div class="banner">
				...
				<span>Flatwise Extra Work Bill Entry</span>                 
				</h2>
...
if(isset($_POST['submit'])){
	$fmon=$_POST['fmonth'].'-01';
$sql="insert into received_entry (flt_sl, date_, for_month,	amount_) values('$_POST[fltsl]', '$_POST[date_]', '$fmon', '$_POST[amount]')";
$res=mysqli_query($db,$sql);

$sql="update month_bill set due=due-'$_POST[amount]' where month_='$fmon' and flt_sl='$_POST[fltsl]'"; 	
$res=mysqli_query($db,$sql);

/*if($res)
{ header('location:set_staff.php');}*/
echo "<strong>Data Save Successful</strong>";
}

/*if(isset($_POST['editsend'])){	
$sql="update expenditure_dtail set date_='$_POST[date_]', exp_sl='$_POST[ecpndsl]',	detail='$_POST[detail]',	amount_='$_POST[amount]' where exdsl='$_GET[sln]'";
$res=mysqli_query($db,$sql);
echo "<strong>Data Edit Successful</strong>";
}*/

$sq= mysqli_query($db,"Select * from received_entry where rec_sl='$_GET[sln]'"); $expdtl=mysqli_fetch_assoc($sq);
?>	
<form class="form-horizontal" method="post">
 <div class="form-group">
    <label  class="col-sm-2 control-label hor-form">Date</label>
    <div class="col-sm-10">     
		<input type="date"  class="form-control" value="<?php echo $expdtl['date_']; ?>" name="date_" >
    </div>
    </div>
	<div class="form-group">
    <label  class="col-sm-2 control-label hor-form">Flat No.</label>
    <div class="col-sm-10">     
		<select  name="fltsl" required class="form-control">
        	
            <?php
$sqn= mysqli_query($db,"Select * from flat_info where exp_sl='$expdtl[exp_sl]'"); $expn=mysqli_fetch_assoc($sqn);
echo "<option value='$expn[flt_sl]'>$expn[flat_no]</option>";
    ...
    <div class="form-group">
    <label  class="col-sm-2 control-label hor-form">For Month</label>
    <div class="col-sm-10">     
		<input type="month"  class="form-control" name="fmonth" value="<?php echo $expdtl['detail']; ?>" placeholder="Detail">
    </div>
    </div>
    <div class="form-group">
    <label  class="col-sm-2 control-label hor-form">Details</label>
    <div class="col-sm-10">     
		<input type="text" required class="form-control" name="amount" value="" placeholder="Details">
    </div>
    </div>
    <div class="form-group">
    <label  class="col-sm-2 control-label hor-form">Amount</label>
    <div class="col-sm-10">     
		<input type="text" required class="form-control" name="amount" value="<?php echo $expdtl['amount_']; ?>" placeholder="Amount">
    </div>
    </div>

  <div class="form-group">
    <div class="col-sm-offset-2 col-sm-10">
      <?php if($_GET['sln']==''){?><button type="submit" name="submit" class="btn btn-success">Save</button><?php } ?>
      <?php if($_GET['sln']!=''){?><button type="submit" name="editsend" class="btn btn-danger">Edit</button><?php } ?>
    </div>
  </div>
