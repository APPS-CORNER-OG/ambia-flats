...
<form class="form-horizontal" method="post">
    ...
    <div class="col-sm-10">     
		<input type="date" required  name="smonth"> <!--class="form-control"-->
    </div>
    </div>

    <div class="form-group">
    <label  class="col-sm-2 control-label hor-form">Income Head</label>
    <div class="col-sm-10">     
		<select name="inchead">
			<option></option>
			
			<?php $res= mysqli_query($db,"Select * from income_head order by inc_head ASC");
			while($icml=mysqli_fetch_assoc($res)){ 			
							echo "<option value='$icml[incsl]'>$icml[inc_head]</option>";
						  }
						  ?>
...
<div class="form-group">
    ...
    <div class="form-group">
    <label  class="col-sm-2 control-label hor-form">Remarks</label>
    <div class="col-sm-10">     
		<input type="text"   name="remak"> <!--class="form-control"-->
    </div>
    </div>
...
<?php
if(isset($_GET['d'])){ mysqli_query($db,"delete  from income_dtail where insl='$_GET[d]'");
...
$sql="insert into income_dtail (date_, incsl, detail_, amount_) values('$_POST[smonth]', '$_POST[inchead]', '$_POST[remak]', '$_POST[cbill]')"; mysqli_query($db,$sql);				
...
<tr><td><strong>Date</strong> </td> <td><strong>Item</strong> </td><td><strong>Detail</strong> </td> <td><strong>Amount</strong></td> <td> </td></tr>				    
</thead>

															<tbody>
<?php
$res= mysqli_query($db,"Select * from income_dtail order by date_ DESC");
			while($row=mysqli_fetch_assoc($res)){ 			
$sq= mysqli_query($db,"Select * from income_head where incsl='$row[incsl]'"); $mslet=mysqli_fetch_assoc($sq);
//$date=date_create("$row[month_]");
//$m= date_format($date,"F-Y");
$tot=$tot+$row['amount_'];
echo "<tr> <td>$row[date_]</td> <td>$mslet[inc_head]</td> <td>$row[detail_]</td> <td align='right'>$row[amount_]</td>  <td> <!--<a href='ent_incm.php?d=$row[insl]'>Delete</a>--> </td>  </tr>";
						  }
//echo "<strong>Data Save Successful</strong>";

echo "<tr> <td></td> <td></td> <td>Total</td> <td align='right'>$tot</td>  <td> <!--<a href='ent_incm.php?d=$row[insl]'>Delete</a>--> </td>  </tr>";
?>
</tbody>
</table>
